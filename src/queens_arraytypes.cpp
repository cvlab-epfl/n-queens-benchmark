...
/*
The template allows us to use the same function for heap std::vectors and stack std::array containers.
There are 2 type arguments ArrayTypeCol, ArrayTypeDg because when using fixed-size std::arrays, the size is part of the type:
std::array<int, n> and std::array<int, 2*n> are different types.
*/
template<typename ArrayTypeCol, typename ArrayTypeDg>
/*
...
@param dg1: TODO
*/
int allQueensAux(int n, int i, ArrayTypeCol& col, ArrayTypeDg& dg1, ArrayTypeDg& dg2)
{
	if (n == i) {
...
int allQueensCol(int n, int j) {
	...
	*/
	...
	std::vector<bool> dg1(2*n, true);
	std::vector<bool> dg2(2*n, true);

#elif ARRAY_NEW
	// std::unique_ptr<bool> col_p(new bool[n]);
	// std::unique_ptr<bool> dg1_p(new bool[2*n]);
	// std::unique_ptr<bool> dg2_p(new bool[2*n]);
	// bool* col = col_p.get();
	// bool* dg1 = dg1.get();
	// bool* dg2 = dg2.get();
	...
	for(int i = 0; i < n; i++) {
		col[i] = true;
		dg1[2*i] = dg1[2*i+1] = true;
		dg2[2*i] = dg2[2*i+1] = true;
	}
#else
	/*
	...
	*/
	...
	int nsol = allQueensAux(n, 1, col, dg1, dg2);
